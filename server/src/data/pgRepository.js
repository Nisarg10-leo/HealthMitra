...
export const pgRepository = {
  users: {
    findById: async (id) => one(await query('SELECT * FROM users WHERE id = $1', [id])),
  ...
  links: {
    forCaregiver: async (caregiverId) => rows(await query('SELECT * FROM patient_caregiver_links WHERE caregiver_id = $1', [caregiverId])),
    forPatient: async (patientId) => rows(await query('SELECT * FROM patient_caregiver_links WHERE patient_id = $1', [patientId])),
    find: async (patientId, caregiverId) => one(await query('SELECT * FROM patient_caregiver_links WHERE patient_id = $1 AND caregiver_id = $2', [patientId, caregiverId])),
  ...
  medications: {
    findById: async (id) => one(await query('SELECT * FROM medications WHERE id = $1', [id])),
    forPatient: async (patientId) => rows(await query('SELECT * FROM medications WHERE patient_id = $1', [patientId])),
    insert: async (item) => one(await query(...Object.values(buildInsert('medications', item)))),
    update: async (id, changes) => one(await query(...Object.values(buildUpdate('medications', id, changes)))),
    remove: async (id) => { const r = await query('DELETE FROM medications WHERE id = $1', [id]); return r.rowCount > 0; }
  },

  doseLogs: {
    findById: async (id) => one(await query('SELECT * FROM dose_logs WHERE id = $1', [id])),
    ...
    exists: async (medicationId, scheduledTime) => {
      const r = await query('SELECT 1 FROM dose_logs WHERE medication_id = $1 AND scheduled_time = $2::timestamptz LIMIT 1', [medicationId, scheduledTime]);
      return r.rowCount > 0;
    },
    insert: async (item) => one(await query(...Object.values(buildInsert('dose_logs', item)))),
    update: async (id, changes) => one(await query(...Object.values(buildUpdate('dose_logs', id, changes))))
  },

  alerts: {
    findById: async (id) => one(await query('SELECT * FROM alerts WHERE id = $1', [id])),
    ...
    update: async (id, changes) => one(await query(...Object.values(buildUpdate('alerts', id, changes))))
  },

  notifications: {
    forUser: async (userId) => rows(await query('SELECT * FROM notifications WHERE user_id = $1 ORDER BY created_at DESC LIMIT 30', [userId])),
    findForUser: async (id, userId) => one(await query('SELECT * FROM notifications WHERE id = $1 AND user_id = $2', [id, userId])),
    insert: async (item) => one(await query(...Object.values(buildInsert('notifications', item)))),
    update: async (id, changes) => one(await query(...Object.values(buildUpdate('notifications', id, changes))))
  },

  sosEvents: {
    findById: async (id) => one(await query('SELECT * FROM sos_events WHERE id = $1', [id])),
    forPatient: async (patientId) => rows(await query('SELECT * FROM sos_events WHERE patient_id = $1 ORDER BY triggered_at DESC', [patientId])),
    insert: async (item) => one(await query(...Object.values(buildInsert('sos_events', item)))),
    update: async (id, changes) => one(await query(...Object.values(buildUpdate('sos_events', id, changes))))
  },

  contacts: {
    forPatient: async (type, patientId) => {
      const table = type === 'doctors' ? 'doctors' : 'chemists';
      return rows(await query(`SELECT * FROM ${table} WHERE patient_id = $1`, [patientId]));
    },
